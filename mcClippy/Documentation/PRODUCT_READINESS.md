...
| Sensitive handling | In progress | Concealed/transient and password-manager markers force sensitive masking instead of skipping capture. Sensitive previews are stored as placeholders and revealed from decrypted data only in memory. |
...
| Onboarding | In progress | First-run window explains shortcut, encryption, sensitive masking, password capture, and exclusions. |
| Settings | In progress | General, Exclusions, and About tabs exist. History max count, regular/pinned retention, max item size, launch at login, and clear controls are now present. Needs final visual polish. |
...
- Password-manager and concealed-marker detection is best-effort. It should remain conservative, masked by default, and easy to extend.
